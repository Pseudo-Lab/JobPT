...
def _yx_in_bounding_boxes(yx, bounding_boxes):
    ...
    top, left, height, width = [jnp.squeeze(v, axis=-1) for v in jnp.split(bounding_boxes, 4, axis=-1)]
...
def _check_tap_actions_match(
    ...
    # distance.
    within_threshold = jnp.linalg.norm(jnp.array(tap_1_yx) - jnp.array(tap_2_yx)) <= matching_tap_distance_threshold_screen_percentage
...
def check_actions_match(
    ...
    drags_match = _check_drag_actions_match(action_1_touch_yx, action_1_lift_yx, action_2_touch_yx, action_2_lift_yx)
